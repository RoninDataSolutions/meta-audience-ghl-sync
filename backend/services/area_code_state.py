"""
NANPA area code → US state lookup.

Used as the primary geographic signal for GHL contacts, since GHL's list
endpoint doesn't return address fields. Phone area codes are tied to state
on number issuance and remain the most reliable cheap signal.

Notes:
- Toll-free area codes (800, 888, 877, 866, 855, 844, 833, 822) map to None.
- Some area codes cross state lines or were reassigned; in those cases we
  map to the dominant state.
- Canadian / Caribbean NANPA codes map to None (out of scope for US heatmap).
- Data current as of late 2025; minor errors won't materially affect the heatmap.
"""
...
AREA_CODE_TO_STATE: dict[str, str] = {
    ...
    "714": "CA", "738": "CA", "747": "CA", "760": "CA", "805": "CA", "818": "CA",
    "820": "CA", "831": "CA", "840": "CA", "858": "CA", "909": "CA", "916": "CA",
...
def state_from_phone(phone: str | None) -> str | None:
    """Extract a 3-digit area code from a US phone number and map it to a state code."""
    if not phone:
        return None
    ...
    if not digits:
        return None
    # Trim leading country code 1 if present and the length looks like a US number
